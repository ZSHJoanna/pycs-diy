...
def opt_magshift(lcs, rawdispersionmethod, verbose=True, trace=False):
    ...
    minout = spopt.fmin_powell(errorfct, inipars, full_output=True, disp=verbose)
    popt = minout[0]
    if popt.shape == ():
        popt = np.array([popt])
    ...
    while True:
        ...
        for l in lcs:
            if l.ml is not None:
                ...
                if loopn == 1:
                    nomllcs = [x for x in lcs if x.ml is None]  # be careful not to use the variable l ...
                    if len(nomllcs) == 0:
                        if verbose:
                        ...
                        if verbose:
                            ...
                            print(
                                ", ".join(["(%s, %s)" % (couple[0].object, couple[1].object) for couple in couplelist]))
                ...
                if l.ml.mltype == "spline":
                    ...
                    minout = spopt.fmin_powell(errorfct, inipars, full_output=True, disp=verbose, maxiter=maxpowellit)
                    popt = minout[0]
                    if popt.shape == ():
                        popt = np.array([popt])
...
def opt_ts_mix(lcs, rawdispersionmethod, movefirst=False, verbose=True):
    ...
    * from this last point, simplex minimization
    ...
    def errorfct(timeshifts):
        ...
        pycs3.gen.lc_func.settimeshifts(lcsc, timeshifts, includefirst=movefirst)
        d2values = np.array([rawdispersionmethod(*couple)["d2"] for couple in couplelist])
        ret = np.mean(d2values)
        return ret

    initshifts = pycs3.gen.lc_func.gettimeshifts(lcsc, includefirst=movefirst)
    if verbose:
        print("Initial time shifts : ", initshifts)

    res = spopt.basinhopping(errorfct, initshifts, niter=100, T=1.0, stepsize=10.0, interval=50, disp=False,
                             niter_success=10)
    ...
    res = spopt.brute(errorfct, bruteranges(3, 2, ann_res), full_output=False, finish=None)
    ...
    res = spopt.brute(errorfct, bruteranges(1, 2, res), full_output=False, finish=None)
    if verbose:
        print("Brute 2 shifts : %s" % res)
        print("Brute 2 dispersion : %7.3f" % errorfct(res))

    res = spopt.brute(errorfct, bruteranges(0.4, 2, res), full_output=False, finish=None)
    if verbose:
        print("Brute 3 shifts : %s" % res)
        print("Brute 3 dispersion : %7.3f" % errorfct(res))

    res = spopt.brute(errorfct, bruteranges(0.1, 2, res), full_output=False, finish=spopt.fmin)
    ...
    pycs3.gen.lc_func.settimeshifts(lcs, res, includefirst=movefirst)
