...
class Data:
	...
	def error_relative(self):
		...
		return the error between the true time delay and the estimated time delay
		"""
		return self.error()/(0.5*error_up+0.5*error_down)	
...
def summed_stats(data: Data, number_pair: int, print_txt: bool = True):
	...
	if print_txt == True :
		...
		print("chi2 \t\t", chi2)
		print("X \t\t", X, "\n")
	return np.array([f, precision, accuracy, chi2, X])

def create_Data(data_name: str, path: str, full_truth: np.array, sample: np.array, name: str, name_type: str, work_dir: str = './'):
	"""
	...
	for i in sample:
		...
		tmp = pickle.load(open(Simulation_directory + lens_name[-1] + "_ECAM" + path,'rb'))
...
def load_Datalist(sample_name: str, full_truth: np.array, samplelist: np.array, name: str, name_type: str, number_pair: int, work_dir: str = './'):
	"""
	...
	#create the data
	spl_sigma_0 = create_Data("spline marg (sigma = 0)", path_spl_sigma_0, full_truth, samplelist[0], name, name_type)
	spl_sigma_05 = create_Data("spline marg (sigma = 0.5)", path_spl_sigma_05, full_truth, samplelist[1], name, name_type)
	spl_sigma_1000 = create_Data("spline marg (sigma = 1000)", path_spl_sigma_1000, full_truth, samplelist[2], name, name_type)
	...
	regdiff_sigma_0 = create_Data("regdiff marg (sigma = 0)", path_regdiff_sigma_0, full_truth, samplelist[7], name, name_type)
	regdiff_sigma_05 = create_Data("regdiff marg (sigma = 0.5)", path_regdiff_sigma_05, full_truth, samplelist[8], name, name_type)
	regdiff_sigma_1000 = create_Data("regdiff marg (sigma = 1000)", path_regdiff_sigma_1000, full_truth, samplelist[9], name, name_type)
	both_baseline = create_Data("marg spline-regdiff (baseline)", path_both_baseline, full_truth, samplelist[10], name, name_type)
...
def create_silver_sample(full_truth: np.array, sample: np.array, data: Data, chi2: bool):
	...
	if chi2 == True :
		to_reject = np.append(np.append(sample[np.abs(truth)>=100], sample[data.chi2()>=3]), sample[data.precision()>=0.40]) 
	else :
...
def create_multiple_silver_sample(full_truth: np.array, sample: np.array, datalist: np.array):
	...
	SSlist[7], SSlist[9] = SSlist[8], SSlist[8] # set the same cut for all the regdiff_spline
...
def plot_delay(sample_name: str, data: Data, figure_directory: str):
	...
	"""
	fig, ax = plt.subplots()
	ax.errorbar(data.lens, data.median, 0.5*(data.error_up+data.error_down), marker = '.', linestyle='none', label = 'Simulated time delay')
...
def plot_SNR(sample_name: str, data: Data, figure_directory: str, name: str, name_type: str, work_dir: str):
	...
	ax1.plot(SNR, precision*100, marker='.', linewidth=0, label="Pearson's coef = %f, p-value = %f"%scipy.stats.pearsonr(SNR,precision))
	ax1.legend()
	...
	ax2.plot(SNR, accuracy*100, marker='.', linewidth=0, label="Pearson's coef = %f, p-value = %f"%scipy.stats.pearsonr(SNR,accuracy))
	ax2.legend()
	...
	ax3.plot(SNR, chi2, marker='.', linewidth=0, label="Pearson's coef = %f, p-value = %f"%scipy.stats.pearsonr(SNR,chi2))
	ax3.legend()
...
def plot_ML(sample_name: str, data: Data, figure_directory: str, name: str, name_type: str, work_dir: str):
	...
	fig1, ax1 = plt.subplots()
	ax1.plot(ML, precision*100, marker='.', linewidth=0, label="Pearson's coef = %f, p-value = %f"%scipy.stats.pearsonr(ML,precision))
	ax1.legend()
	ax1.set_ylabel('Precision [%]')
	...
	fig2, ax2 = plt.subplots()
	ax2.plot(ML, accuracy*100, marker='.', linewidth=0, label="Pearson's coef = %f, p-value = %f"%scipy.stats.pearsonr(ML,accuracy))
	ax2.legend()
	ax2.set_ylabel('Accuracy [%]')
	...
	fig3, ax3 = plt.subplots()
	ax3.plot(ML, chi2, marker='.', linewidth=0, label="Pearson's coef = %f, p-value = %f"%scipy.stats.pearsonr(ML,chi2))
	ax3.legend()
	ax3.set_ylabel('$\chi^2$ [ ]')
